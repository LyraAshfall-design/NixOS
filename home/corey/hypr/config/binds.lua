...
-- Conventional direct workspace keys
local workspaceKeycodes = { 10, 11, 12, 13, 14, 15 }

for workspace = 1, 6 do
    local digit = tostring(workspace)
    local keycode = "code:" .. tostring(workspaceKeycodes[workspace])

    -- Remove existing number-key assignments
    hl.unbind(mainMod .. " + " .. digit)
    hl.unbind(mainMod .. " + SHIFT + " .. digit)
    hl.unbind(mainMod .. " + " .. keycode)
    hl.unbind(mainMod .. " + SHIFT + " .. keycode)

    hl.bind(
        mainMod .. " + " .. keycode,
        hl.dsp.focus({ workspace = digit })
    )

    hl.bind(
        mainMod .. " + SHIFT + " .. keycode,
        hl.dsp.window.move({ workspace = digit, follow = true })
    )
end

hl.bind("SUPER + ALT +W",
hl.dsp.exec_cmd("noctalia msg panel-toggle control-center weather"))
